...
def write_values (wins, draws,lost,avg_time,reward,n_games=0):
    ...
    process = Popen(['tail','-n','1','Connect4/TrainningData/games_q_2nd_rand.csv'],stdout=PIPE)
    try:
        last_game = int(process.stdout.readlines())[0].decode('UTF-8').split(',')[2]
    except:
        last_game=0
    with open('TrainningData/games_q_2nd_rand.csv', 'a') as games_file:
        games_file.write(
            f'\n{date_value},{last_game+n_games},{wins},{draws},{avg_time},{lost},{reward}')

qdict = {}
table = pd.read_csv('TrainningData/q_learning_table_2nd_rand.csv')
...
while True:
    ...
    df['scores'] = scores
    df['scores'].sum()
    df.to_csv('Connect4/TrainningData/q_learning_table_2nd_rand.csv', index=False)
